...
static double distanceBetweenTwoPoints(Point2d pt1, Point2d pt2) {
...
static double Rand() { return rand() % 100; }
static double RandUnity() { return (double)rand() / RAND_MAX; }

static bool sortByEdgelength(const pair<pair<Point2d, Point2d>, double> &edge1, const pair<pair<Point2d, Point2d>, double> &edge2) {
    return (edge1.second < edge2.second);
}

//Generating random points on the given graph's edges.
static void randomPoint(const double& totalLength, vector<double>& edgesLinearInterval, EdgeLinear& edgesLinear, Graph& graph, Mat const& mapperImage, VectorPoint2d& RandomPoints) {
...
//Developing a Cloud of random points.
static void euclideanCloud(Graph& cloud, VectorPoint2d RandomPoints, int randomCloudSize, Mat& cloudImage) {
...
static void euclideanGraph(Graph& graph, VectorPoint2d& RandomPoints, int randomCloudSize, Mat& mapperImage) {
    srand(time(NULL));
    VItr vitr, vend;
...
//Normalization of the boundaries of the image as per the size of Graph.
static void Bounding_Box_Graph(Graph const& graph, std::pair<Point2d, Point2d>& graph_box) {
...
static void Bounding_Box_Cloud(Graph const& cloud, std::pair<Point2d, Point2d>& cloud_box) {
...
static void Scaling_Parameters_Graph(Graph const& graph, Point image_sizes, double& scale, Point2d& shift) {
...
static void Scaling_Parameters_Cloud(Graph const& cloud, Point image_sizes, double& scale, Point2d& shift) {
    std::pair<Point2d, Point2d> box;
...
static void Draw_Graph(Graph const& graph, int radius, Scalar color, Mat& image, double& scale, Point2d& shift, String name) {
...
static void neighbourHoodGraph(Graph& randomGraph, double threshold) {
...
static void Draw_Component(Graph const& components, int radius, Scalar color, Mat& image, double& scale, Point2d& shift, String name) {
...
//Finding the number of Components present in the generated Random Graph.
static void findNoOfComponents(const Graph& randomGraph, const int& vertexCount, double& scaleGraph, Point2d& shiftGraph, vector<Graph>& graphVector, Mat& coloredCloudImage, double threshold) {
    ...
    for (iItr = 0; iItr<componentCount; iItr++) {
        ...
        tempGraph.clear();//For clearing the non-required memory.
        jItr = 0;
    }
}
