...
static void write_image(String const& name, Mat const& mapperImage) {
    try {
...
static double distanceBetweenCoordinates(dataPoint pt1, dataPoint pt2) {
...
static void calculateGaussDensity(vector<dataPoint>& points, double sigma) {
...
static bool sortByGauss(const pair<double, Point2d>& a,
                 const pair<double, Point2d>& b) {
    return (a.first < b.first);
}

static void colormap(std::vector<dataPoint>& points,
...
static void drawingCloud(float scalingFactor,
...
static void scalingCloud(vector<dataPoint>& points, int size) {
