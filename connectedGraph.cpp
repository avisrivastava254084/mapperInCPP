...
void neighbourHoodGraph(Graph& randomGraph, double threshold, VectorPoint2d& connectedComponents) {
	...
	for (ep = edges(randomGraph); ep.first != ep.second; ++ep.first) {
		...
		if ( vertexOne == vertexTwo ) {
			...
			vertexTwo = vertexOne;
			connectedComponents.push_back(point/count);
...
void Bounding_Box(VectorPoint2d const& cloud, std::pair<Point2d,
	Point2d>& cloud_box) {
	...
	for (auto p : cloud) {
		...
		if (cloud_box.first.x < p.x) cloud_box.first.x = p.x;

		if (cloud_box.first.y < p.y) cloud_box.first.y = p.y;

		if (cloud_box.second.x > p.x) cloud_box.second.x = p.x;

		if (cloud_box.second.y > p.y) cloud_box.second.y = p.y;
...
void Scaling_Parameters(VectorPoint2d const& cloud, Point
...
void Draw_Cloud(VectorPoint2d const& cloud, int radius, Scalar
	...
	for (auto p : cloud)
		...
		cv::circle(mapperImage, Point(scale * p.x + shift.x, scale * p.y + shift.y), radius, color, -1);

	write_image("newImage.png", mapperImage);
}
...
int main() {
	...
	//Draw_Cloud(g, 5, CV_RGB(255, 0, 255), cloudImage, scaleGraph, shiftGraph, "scaledRandomCloudGraph.png");
	Draw_Cloud(connectedComponent, 6, CV_RGB(255, 0, 255), graphImage);
}
