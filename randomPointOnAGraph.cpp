...
void randomPoint(const double& totalLength, vector<double>& edgesLinearInterval, EdgeLinear& edgesLinear, Graph& g, Mat const& mapperImage, VectorPoint2d& RandomPoints){
	double linearPoint, t; int noOfEdge = 0; int check = 0;
	linearPoint = RandUnity() * totalLength;
	if(linearPoint == 0) { linearPoint += RandUnity(); }
	cout<<endl<<"The random linear point is:"<<linearPoint<<endl; noOfEdge = 0;
	check = 0 ;
...
void euclideanGraph(Graph& g, VectorPoint2d& RandomPoints, int randomCloudSize, Mat& mapperImage) {
  srand(time(NULL));
  VItr vitr, vend;
...
int main() {
    ...
    euclideanGraph(g, RandomPoints, randomCloudSize, graphImage);
