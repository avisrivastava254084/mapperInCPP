...
void Draw_Cloud (std::vector<Data>const& cloud, Mat& image, Graph& g) {
...
void Cloud_On_Circles(vector<Data>& cloud){
	int cloudsize= cloud.size();
		      
	     	for(int i=0; i<num_of_points; i++){
		    double Angle = Rand()* 2 * pi;
		    cloud[i].point.x =cos(Angle);
		    cloud[i].point.y =sin(Angle);
...
int main() {
	...
	Cloud_On_Circles(cloud);
	Pertube_Cloud(cloud, 0.0);
    distanceCheck(cloud, g);
    Draw_Cloud(cloud, image, g); 
    String perturbedCloud = "NeighborCloud";
