...
void readPoints(vector<dataPoint>& coordinate_points) {
	ifstream input;
	string filename;
	...
	while (input >> myPoint.pt.x >> ch >> myPoint.pt.y) {
		...
		coordinate_points.push_back(myPoint);
...
void colormap(std::vector<dataPoint>& coordinate_points,
	...
	int pointssize = coordinate_points.size();
	int colorsize = colormap.size() - 1;
	float filter_iterator = (float)pointssize / (float)colorsize;
	int filter_function_iterator = ceil((float)filter_iterator);
	vector<pair<double, Point2d> > fdp;
	for (vector<dataPoint>::iterator it = coordinate_points.begin(); it != coordinate_points.end();
		it++) {
	...
	for (vector<pair<double, Point2d> >::iterator it = fdp.begin();
		...
		if (i %  filter_function_iterator == 0) {
			lBound = fdp[i].first;
			//  cout<< lBound <<endl;
			if ((i + filter_function_iterator) < pointssize) {
				rBound = fdp[(i + (filter_function_iterator - 1))].first;
			}
			else {
				rBound = fdp[(pointssize - 1)].first;
			}
		}
		...
		if (i %  filter_function_iterator == 0) {
			clBound = colormap[i / filter_function_iterator];
			if (((i / filter_function_iterator) + 1) < colormap.size()) {
				crBound = colormap[((i / filter_function_iterator) + 1)];
			}
			else {
...
void scalingCloud(vector<dataPoint>& coordinate_points, int size) {
	vector<dataPoint> copyPoints(coordinate_points);
	dataPoint currentVector;
...
int main() {
	vector<dataPoint> coordinate_points;
	readPoints(coordinate_points);  // input taken as a point cloud.
						 // calculateEccentricity(points);
	calculateGaussDensity(coordinate_points, 7.0);
	String scaledCloud = "test";
	cout << "Enter the size of the image to be saved" << endl;
	int imageSize;
	cin >> imageSize;
	scalingCloud(coordinate_points, imageSize);
