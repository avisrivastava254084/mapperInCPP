...
void Perturb_Cloud (double noise_bound, std::vector<dataPoint>& cloud, double scalingFactor, int imageSize, Mat mapperImage) {
	
	Point2d center;
	cout<<"The image size ka half is:"<<(imageSize/2)<<endl;
    for ( int i = 0; i < cloud.size(); i++) {
           cloud[i].pt.x = (cloud[i].pt.x + (noise_bound * scalingFactor) + (imageSize/2));
           cloud[i].pt.y = (cloud[i].pt.y + (noise_bound * scalingFactor) + (imageSize/2));
...
void randomCloudNearCircle(double noiseBound, int imageSize, int cloudSize){
	...
	for(int radii = 1; radii < 6; radii++){
	for(int i = 0; i<cloudSize; i++){
		double shiftAngle = Rand()* 2 * pi;
		cloud[i].pt.x = (radii * (1+cos(shiftAngle)));
		cloud[i].pt.y = (radii * (1+sin(shiftAngle)));
	...
	//Perturb_Cloud(noiseBound, cloud, scalingFactor, imageSize, mapperImage);
}
