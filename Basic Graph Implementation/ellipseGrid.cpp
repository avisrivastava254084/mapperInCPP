...
void euclideanVertices(Graph& g, int size_m, int size_n) {
...
void Draw_Cloud (Graph const& cloud, int radius, Scalar color, Mat& image, double& scale, Point2d& shift,int n,int m) {
	...
	Point2d p1 = midpoint(end_points[0],end_points[1]);
	Point2d p2 = midpoint(end_points[2],end_points[3]);
	ellipse(image,midpoint(p1,p2),Size(euclideanDist(end_points[0],end_points[3])*0.5 + euclideanDist(end_points[0],end_points[3])*0.125,euclideanDist(end_points[2],end_points[3])*0.5 + euclideanDist(end_points[2],end_points[3])*0.125),0,0,360,CV_RGB(0,0,255),1,8);	
